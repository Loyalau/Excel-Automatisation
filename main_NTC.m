...
G_NTC = table(Size=[Number_test_EN 1], VariableTypes="double", VariableNames="Play_[mm]"); % Table that will receive later all the play (Gioco in italian) of all the test
for i = 1 : Number_test_NTC
    G_NTC{i, 1} = input(sprintf('Enter the measured Play for the test number %d : ', i));
end 
Nb_Cycle_NTC = [10]; % Here is the number of cycle for the NTC18 procedure
